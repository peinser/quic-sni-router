...
| Source-port-reuse misrouting | Kernel reuses an ephemeral UDP source port quickly after a previous connection closes. A fresh QUIC Initial arrives on a tuple that matches a stale forward-alias from the closed connection, and is misrouted to the previous backend. Caught by the loadtest in `tests/e2e/loadtest/`. | Initial packets are routed by their (DCID, SCID) pair-key first; a tuple match alone is only honoured for non-Initial packets. Regression net: `tests/e2e/loadtest/` asserts zero misroutes |
| Backend return-path ambiguity | One backend UDP socket serves many QUIC connections, and backend-to-client packets may carry no routable bits at all (zero-length client CIDs, post-handshake rotated CIDs, stateless resets). Any shared return channel can deliver one client's packets to another. | Every client tuple gets a dedicated upstream socket; backend return traffic is demultiplexed by receiving fd, with no dependence on packet contents. `tests/e2e/flows/` asserts the isolation; persistent and fresh-connection loadtests compare router vs direct backend behaviour. |
| Flow-socket exhaustion | One upstream socket per concurrent client flow; a flood of new client tuples can hit `RLIMIT_NOFILE`. | Flow table is bounded (`maxSessions / 4`) with LRU eviction; on `EMFILE`/`ENFILE` the oldest flow is recycled and the open retried. `RLIMIT_NOFILE` is raised to the hard limit at startup with a warning when it cannot cover the flow table. Combine with upstream per-source rate limits |
...
| Backend-spoofing return traffic | Attacker on WAN sends packets with source = backend IP | Flow sockets accept datagrams from any source (tolerating Kubernetes Service/pod NAT asymmetry), so an attacker must spoof the backend's address AND guess a live flow's ephemeral port to inject toward that flow's client. Mitigated by binding backends to non-routable / cluster-internal addresses and by QUIC's own AEAD on the client side |
...
- Backend return traffic is demuxed by per-client upstream flow sockets (the receiving fd identifies the client), eliminating shared-backend-tuple confusion under concurrency regardless of what CIDs the packets carry.
...
- No ECH inner-SNI visibility: with Encrypted ClientHello, the router sees only the outer ClientHello's cover hostname and routes based on that. Routing to the real inner hostname is impossible without terminating TLS (which we don't). Operational impact: ECH-using clients work iff the cover hostname is a configured route; otherwise they drop. Detection of ECH presence (for observability counters) is in [ROADMAP.md](../ROADMAP.md), but routing semantics will not change without TLS termination.

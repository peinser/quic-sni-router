...
| Source-port-reuse misrouting | Kernel reuses an ephemeral UDP source port quickly after a previous connection closes. A fresh QUIC Initial arrives on a tuple that matches a stale forward-alias from the closed connection, and is misrouted to the previous backend. Caught by the loadtest in `tests/e2e/loadtest/`. | Initial packets are routed by their (DCID, SCID) pair-key first; a tuple match alone is only honoured for non-Initial packets, or for Initials whose tuple-matched destination is NOT a configured backend (a reverse alias for a backend's Initial response). Regression net: `tests/e2e/loadtest/` asserts zero misroutes |
| Backend reverse-tuple ambiguity | One backend UDP socket serves many QUIC connections. A reverse alias keyed only by `backend_ip:port` can be overwritten by another connection, causing backend packets to be sent to the wrong client and rejected by QUIC AEAD. | Packets from configured backends are routed by learned long-header/short-header DCID aliases before backend tuple fallback. Persistent and fresh-connection loadtests compare router vs direct backend behaviour. |
...
| Backend-spoofing return traffic | Attacker on WAN sends packets with source = backend IP | The router may forward packets whose source tuple matches a learned backend tuple or whose visible DCID matches a learned client alias. Mitigated by binding backends to non-routable / cluster-internal addresses and by QUIC's own AEAD on the client side |
...
- Backend return traffic is demuxed by learned DCID aliases before falling back to the backend tuple, avoiding shared-backend-tuple confusion under concurrent handshakes.

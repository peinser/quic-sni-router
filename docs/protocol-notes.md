...
For production diagnosis, set `QSR_DEBUG_PACKETS=1` on the router pod. It logs one line per forwarded/dropped packet with packet kind, source tuple, backend-source classification, selected destination, and routing decision (`fresh_sni`, `tuple`, `short_cid_rebind`, `drop_initial_route`, etc.). Do not leave this enabled under normal traffic volume.

...
- **Backend connection logging** (`logging.connections: true`, default off, hot-reloadable): one stderr line per backend flow establishment (`conn: open src=<ip:port> sni=<name> scid=<hex> backend=<ip:port>`) and one per teardown with a close reason (`idle`, `evicted`, `reload`, `shutdown`, `reroute`, `error`) and the flow duration. Implemented as a close hook on the flow table plus per-flow metadata (SNI, client Initial SCID, open timestamp) stamped once at flow creation; the per-packet forwarding paths are untouched, so the feature has no packet-rate cost whether enabled or not. Exposed in the Helm chart as `config.logging.connections`.

### Fixed

- **Hot reload silently dropped `cidEncoding` changes**: `reload_from_path` swapped routes and idle timeout but never copied the freshly parsed CID codec, so rotating (or enabling/disabling) the encoded-CID key required a process restart while the log claimed `reload: ok`. The codec now swaps atomically with the routes and the reload log announces the change. `logging.connections` is hot-swapped the same way.

- **Concurrent connections to one backend (per-flow upstream sockets)**: the single-socket dataplane could not correctly demultiplex backend return traffic for more than one concurrent QUIC connection per backend whenever the packets carried no routable bits: clients using zero-length connection IDs (Chrome does), post-handshake rotated CIDs (NEW_CONNECTION_ID is encrypted and invisible to a pass-through router), and stateless resets all fell back to a single shared `backend_ip:port -> client` reverse tuple, which only ever points at the most recent sender. Every client tuple now gets a dedicated unconnected upstream UDP socket (`qsr_flow_table`): the backend sees one router source port per client and return datagrams are routed by the receiving fd, with zero dependence on packet contents. Clients still see all traffic from the router's listen tuple. The flow table is bounded at `maxSessions / 4` with LRU eviction; `RLIMIT_NOFILE` is raised to the hard limit at startup and `EMFILE` degrades to LRU flow recycling. Hot reload hard-cutovers flows whose backend was removed, mirroring session eviction. This supersedes the DCID-first backend return routing and reverse-tuple refresh fixes below, whose mechanisms (backend-source classification, reverse tuple aliases, client-SCID aliases) are removed from the dataplane; per-connection session-table entries shrink from ~5 to ~4. The Linux event loop is now one epoll set over listen + flow + inotify fds with bounded `recvmmsg` drains; the send batcher groups consecutive same-fd datagrams into single `sendmmsg` calls; the non-Linux fallback is a nonblocking `poll()` loop. New `tests/e2e/flows/` asserts two clients to one backend get distinct upstream tuples and that interleaved CID-less return packets reach the right client (`make test-e2e-flows`, wired into CI).
...
- **Established 1-RTT fast path**: a short-header packet from a client tuple with a live flow is forwarded after a single flow-table probe (~17 ns); the session table is no longer consulted and no header fields beyond the long-header bit are inspected. Backend return traffic does zero table lookups per packet (the receiving flow socket identifies the client). Disabled when encoded-CID routing is enabled, which is contracted to route by decoded server_id first. New `flow tuple lookup` entry in `make benchmark`; the persistent loadtest sustains ~9.3k req/s at p50 1.0ms / p99 1.7ms in the reference Docker environment with zero misroutes.

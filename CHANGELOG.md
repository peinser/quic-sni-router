...
- **Post-idle stateless reset routing**: established client-to-backend packets now refresh the backend-to-client reverse tuple before forwarding. This keeps backend stateless resets generated after a backend idle timeout on the client tuple that triggered them, instead of following stale reverse state from an earlier packet.

...
docker run -d --name "${project}-backend" --network "${network}" --network-alias addr-echo-backend \
  "${http3_image}" python /app/addr_echo_backend.py --port 8443 >/dev/null
docker create --name "${project}-router" --network "${network}" --network-alias flows.flightdeck.test \
...
if ! docker run --name "${project}-client" --network "${network}" \
  "${http3_image}" python /app/flows_client.py --router-host "${router_ip}" --router-port 443; then
  dump_logs
  exit 1
fi
